...
//			});
	...
	this.getLimitAndPlanValue = function() {
		var time = $("#time").val();
		var time1 = $("#time").val().replace(/[^0-9]/ig, "");
		$.ajax({
	...
	this.getAllLimitLineData = function(selectedDalare, type,isRefresh) {
		...
		if (isRefresh || ((!myLimitLine.selectedDalare || myLimitLine.selectedDalare.attr('limitLineId') != selectedDalare.attr('limitLineId')) && !myLimitLine.checkDataChanged())) {
			...
			myLimitLine.changeLimitLineTypeStyle("可用容量");
			myLimitLine.selectedDalare = selectedDalare;
			//设置默认值
			if(myLimitLine.limitLineType == undefined){
				myLimitLine.limitLineType = "可用容量";
				myLimitLine.dtype = "可用容量";
			}
			myLimitLine.mcorhr= selectedDalare.attr('limitLineId');
			mask();
			$.ajax({
				...
				success : function(result) {
					if (result != null) {
						...
						myLimitLine.staticData = result['静态正向限额'];
						myLimitLine.dynamicData = result['动态正向限额'];
						myLimitLine.tradeData = result['交易功率'];
						myLimitLine.spaceData = result['可用容量'];
					...
					//alert(myLimitLine.mcorhr+"==1=="+myLimitLine.dtype);
					unmask();
					//alert("ok");
			...
			$.ajax({
				...
				success : function(result) {
					...
					if (result != null) {
						...
						myLimitLine.dtype=type;
						myLimitLine.loadChartData();
						//alert("mcorhr==="+myLimitLine.mcorhr+"===dypte==="+type);
						$('#limitLineDataDiv input[class=datainput]').each(function(){this.value=0;});
						myLimitLine.showLimitLineDataDiv();
					}
					//alert("ok");
	...
	this.refreshLimitLineData = function() {
		...
		this.getAllLimitLineData($(".lmenu").children("ul:first").children(".bghh"),myLimitLine.limitLineType,true);
		//getBtnStatus();
		//this.getAllLimitLineData();
	};
	// 切换限额管理
	...
	this.getLimitLineDataByLimitLineType = function(type) {
		...
		myLimitLine.dtype = type;
		limitLineType = type;
		if(myLimitLine.dtype == "可用容量" || myLimitLine.dtype == "日前计划值"){
			$('#limitLineDataDiv input').attr('readonly', 'readonly');
			$('#btn_save').hide();
		}else{
			$('#limitLineDataDiv input').removeAttr("readonly");
			$('#btn_save').show();
			$('#btn_save').css('color','#06938e');
		}
		...
		if (!myLimitLine.checkDataChanged()) {
			...
			if(type!="可用容量"){
				...
				//console.info(sxData);
				//console.info(jhData);
				if(sxData == null || jhData == null){
					$('#limitLineDataDiv input[name=' + key + ']').val(0);
					$('#limitLineDataDiv span[name=sumValue]').text(0);
					$('#limitLineDataDiv span[name=avgValue]').text(0);
					myLimitLine.showLimitLineDataDiv();
					return ;
				}
				var sum=0;
				for (var key in sxData) {
					if (/^h[0-9]{2}$/.test(key)) {
						$('#limitLineDataDiv input[name=' + key + ']').val($.format(sxData[key]-jhData[key], 3, ','));
						sum += Number(sxData[key]-jhData[key]);
					}
				}
				var avg = sum /96;
				$('#limitLineDataDiv span[name=sumValue]').text($.format(sum, 3, ','));
				$('#limitLineDataDiv span[name=avgValue]').text($.format(avg.toFixed(0), 3, ','));
				//myLimitLine.loadChartData();
				myLimitLine.showLimitLineDataDiv();
			}	
		}
		...
		if(area!='国调'){
			...
			$.ajax({
				...
				success : function(result) {
					...
					if(myLimitLine.limitLineType == "可用容量"){
						limitLine.getLimitLineDataByLimitLineType('可用容量');
					}
	...
	this.makeDataToChart = function(result) {
			...
			var seriesData_beforeData = new Array();
			var seriesData_dayData = new Array();
			var seriesData_staticData = new Array();
			var seriesData_dynamicData = new Array();
			var seriesData_tradeData = new Array();
			var seriesData_spaceData = new Array();
				...
				for(var i = 1;i<97;i++){
					...
					key = "h"+keyStr;
					//data[key] = 0;
					seriesData_beforeData.push(this.beforeData[key] == undefined?0:this.beforeData[key]);
					seriesData_dayData.push(this.dayData[key] == undefined?0:this.dayData[key]);
					seriesData_staticData.push(this.staticData[key] == undefined?0:this.staticData[key]);      
					seriesData_dynamicData.push(this.dynamicData[key] == undefined?0:this.dynamicData[key]);      
					seriesData_tradeData.push(this.tradeData[key] == undefined?0:this.tradeData[key]);      
					seriesData_spaceData.push(this.spaceData[key] == undefined?0:this.spaceData[key]);
				}
			...
			if(_isState){
				seriesArr = [{
					name : '可用容量',
					data : seriesData_spaceData
				},
				{
					name : '日前预计划',
					data : seriesData_beforeData
				}/*,
				{
					name : '日内计划',
					...
					selected:false,
					enabled:false,
					data : _limitType==0?seriesData_dynamicData:seriesData_staticData
				}
				]
			}else{
				seriesArr = [{
...
'22:00','22:15','22:30','22:45','23:00','23:15','23:30','23:45'
	...
	this.makeDataByTable = function() {
		...
		//alert(myLimitLine.mcorhr+"==="+myLimitLine.dtype);
	...
	this.changeLimitLineTypeStyle = function(type) {
		...
		for (var i = 0; i < limitLineTypes.length ; i++) {
	 ...
	 * 本地导出
	 */
	this.localExport = function(){
		window.open(baseUrl+"/lineLimit/localExportData?time="+$('#time').val());
	};
	// 导出通道计划值
