function LimitLine() {
	...
	this.getAllLimitLineData = function(selectedDalare, type,isRefresh) {
		//alert("ccc");
		var time = parent.$("#time").val().replace(/[^0-9]/ig, "");
		var area=$("#area").val();
		var state="";
		if(area!='国调'){
			//state=1;
		...
		//alert(type);
		//console.info(myLimitLine.selectedDalare );
		//console.info(selectedDalare );
			...
			$.ajax({
				...
				success : function(result) {
					...
					if (result != null) {
						...
						myLimitLine.staticData = result['静态限额'];
						myLimitLine.dynamicData = result['动态限额'];
						myLimitLine.tradeData = result['交易功率'];
						myLimitLine.spaceData = result['差值空间'];
	...
	this.getLimitLineData = function(selectedDalare, type) {
		...
		if ((!myLimitLine.selectedDalare || myLimitLine.selectedDalare.attr('limitLineId') != selectedDalare.attr('limitLineId')) && !myLimitLine.checkDataChanged()) {
			...
			myLimitLine.changeLimitLineTypeStyle("差值空间");
			myLimitLine.selectedDalare = selectedDalare;
			//设置默认值
			myLimitLine.limitLineType = "差值空间";
			$.ajax({
				url : 'getLineLimit',
				type : 'POST',
				dataType : 'json',
		...
		if(area!='国调'){
			...
			$.ajax({
				...
				success : function(result) {
					...
					if(myLimitLine.limitLineType == "差值空间"){
						limitLine.getLimitLineDataByLimitLineType('差值空间');
					}
