...
this.loadTree = function() {
	...
	var setting = {
	 ...
	 var zNodes=jQuery.parseJSON(myIssue.TreeData());
						...
						if(isId=="getBillCountMeg"){
							...
							if (result) {
								 content="<thead><th width='7%' class='hdth'>地区</th>"+
									...
									"<th width='10%' class='hdth'>单子总电量(MWh)</th>"+
									"<th width='10%' class='hdth'>出清电量(MWh)</th>"+
									"<th width='10%' class='hdth'>出清费用(元/MWh)</th>"+
									"<th width='10%' class='hdth'>结算电量(MWh)</th>"+
									"<th width='10%' class='hdth'>结算费用(元/MWh)</th></thead>";
								...
								for(var i=0;i<result.length;i++){
								content=content+"<tr><td width='7%' class='isTabNum'>"+result[i].area+"</td>"+
									"<td width='8%' class='isTabNum'>"+result[i].sheetName+"</td>"+
									"<td width='10%' class='isTabNum'>"+result[i].sumQ+"</td>"+
									"<td width='10%'  class='isTabNum'>"+result[i].clear+"</td>"+
					...
					if(isId=="getPriceCountMeg"){
						...
						if (result) {	
							 content="<thead><th width='7%' class='hdth'>地区</th>"+
								...
								"<th width='10%' class='hdth'>出清费用(元/MWh)</th>"+
								"<th width='10%' class='hdth'>结算电量(元/MWh)</th>"+
								"<th width='10%' class='hdth'>结算费用(元/MWh)</th></thead>";
								/*"<th width='10%' class='hdth'>跨区输电费用</th>"+
								"<th width='15%' class='hdth'>从送端累加的总费用</th>"+
								"<th width='10%' class='hdth'>总费用(元/MWh)</th></thead>";*/
							for(var i=0;i<result.length;i++){
							content=content+"<tr><td width='7%' class='isTabNum'>"+result[i].area+"</td>"+
								"<td width='10%' class='isTabNum'>"+result[i].clear+"</td>"+
								"<td width='10%' class='hdth'>/</td>"+
				...
				success : function(result) {
					if (result) {
						 content="<thead><th width='7%' class='hdth'>地区</th>"+
								...
								"<th width='10%' class='hdth'>结算电量(元/MWh)</th>"+
								"<th width='10%' class='hdth'>结算费用(元/MWh)</th></thead>";	
						content=content+"<tr><td width='7%' class='isTabNum'>"+result.count.area+"</td>"+
						"<td width='10%' class='isTabNum'>"+result.count.clear+"</td>"+
						"<td width='10%' class='hdth'>/</td>"+
						"<td width='10%' class='hdth'>/</td>"+
						"<td width='10%' class='hdth'>/</td></tr>";	
						$('#dtc').append(content);
						
						if(result.detail.dtype=='1a'){
							$('#dtype').text('全天');
						}else if(result.detail.dtype=='2a'){
							$('#dtype').text('高峰');
						}else if(result.detail.dtype=='3a'){
							$('#dtype').text('低谷');
						}
